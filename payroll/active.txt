# 01_工资模块（v2025-11-25R65）
版本 v2025-11-25R65
生效：2025-11-25
打印版本 v2025-11-25R65

## 变更说明（R65）
- 支付合并表同时存在报销状态与报销结果时，结果“通过/未通过”均进入待确认，不再触发阻断。
- 待确认原因新增“已通过未支付/未通过”，并在单人汇总与批量汇总索引按原因统计。
- schema 层不对状态值做枚举拦截，状态为空/任意字符串均可进入待确认判定。
...
- 状态为空且结果=通过 → 待确认（原因=已通过未支付）；结果=未通过 → 待确认（原因=未通过）。
...
- 版本尾注固定：计算口径版本 v2025-11-25R65｜阻断模式：Hard。
## 批量出单（项目结算）
