# 01_工资模块（v2026-01-05R76）
版本 v2026-01-05R76
生效：2026-01-05
打印版本 v2026-01-05R76

## 变更说明（R76）
- 固定日薪接入定价链，个人固定日薪覆盖全组/单防撞日单价，并输出命中/未命中审计。
- 同一项目内显示名映射同一 name_key 时 Hard 阻断，输出显示名与行号证据。
...
- 选表规则：清洗表头后按关键词打分；当存在 2 个 CSV 时允许交叉命中差值判定；无法唯一确定则阻断并输出候选得分与表头摘要。
...
- 日薪来源优先级（锁死）：
  - 口令固定日薪 > 系统默认固定日薪映射(7人) > 表内角色日薪 > 角色默认(组长300/组员200) > 兜底。
- 系统默认固定日薪映射（姓名优先）：
  - 王怀宇300；余步云260；董峰300；董祥300；王怀良230；袁玉兵300；马建300。
...
- 版本尾注固定：计算口径版本 v2026-01-05R76｜阻断模式：Hard。
## 批量出单（项目结算）
